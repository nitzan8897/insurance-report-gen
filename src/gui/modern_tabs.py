# src/gui/modern_tabs.py
"""
Modern tab manager that shows only relevant fields based on case type.
"""
...
class ModernTabManager:
    ...
    def __init__(self, parent, data_manager, case_type):
        ...
        # Style notebook tabs
        style = ttk.Style()
        style.configure('TNotebook.Tab', font=('David', 11), padding=[15, 8])
    ...
    def create_additional_tab(self):
        ...
        ttk.Label(scrollable, text='סרטוני וידאו',
                 font=('David', 10)).grid(row=1, column=1, padx=5, pady=5, sticky='e')
        ...
        ttk.Label(scrollable, text='התכתבויות / תמונות',
                 font=('David', 10)).grid(row=2, column=1, padx=5, pady=5, sticky='e')
    ...
    def create_field(self, parent, row, label_text, field_name,
        ...
        ttk.Label(parent, text=label_text,
                 font=('David', 10)).grid(row=row, column=1, padx=5, pady=5, sticky='e')

        # Widget
        if widget_type == 'entry':
            widget = ttk.Entry(parent, width=40, font=('David', 10))
            widget.grid(row=row, column=0, padx=5, pady=5, sticky='ew')

        elif widget_type == 'combo':
            values = kwargs.get('values', [])
            widget = ttk.Combobox(parent, values=values, width=37,
                                 state='readonly', font=('David', 10))
            widget.grid(row=row, column=0, padx=5, pady=5, sticky='ew')

        elif widget_type == 'text':
            height = kwargs.get('height', 5)
            widget = tk.Text(parent, width=40, height=height, font=('David', 10))
            widget.grid(row=row, column=0, padx=5, pady=5, sticky='ew')

        elif widget_type == 'date':
            widget = DateEntry(parent, width=37, background='darkblue',
                             foreground='white', borderwidth=2,
                             date_pattern='dd/mm/yyyy', font=('David', 10))
            widget.grid(row=row, column=0, padx=5, pady=5, sticky='ew')
